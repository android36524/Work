	...
	public void updateDataSources() {
		...
		for (DataSourceVO<?> dataSource : dataSources) {
			if(dataSource instanceof ModbusIpDataSourceVO){
				ModbusIpDataSourceVO ds = (ModbusIpDataSourceVO)dataSource;
				ds.setUpdatePeriods(20);
				dataSourceDao.saveDataSource(dataSource);
				System.out.println("update "+dataSource.getName());
			}
		...
		for (DataSourceVO<?> dataSource : dataSources) {
			...
			if (fid > 292) {
				...
				for (DataPointVO p : points) {
					...
					appSettingDao.addPoint(pointName, p.getId() + "", appacpid+"","0","");
				}
