...
public class AlarmPointStatisticsMsgBody extends MsgBody implements IImage{
	...
	private String buildPoints() {
		...
		if(size==0){sb.append("]");return sb.toString();}
		stime = points.get(size-1).getTime();
		Object[]array = new Object[size];
		for(PointValueTime p:points){
			int index = (int) ((p.getTime()-stime)/DURATION);
			if(index>=size)continue;
			array[index] = p.getDoubleValue();
		}
		for(Object obj:array){
			String s = obj!=null?StringUtil.formatNumber((Double)obj, "0.00")+",":"-1,";
			sb.append(s);
		}
