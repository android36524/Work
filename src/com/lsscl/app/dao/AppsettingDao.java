...
public class AppsettingDao extends BaseDao {
	...
	private static final String Point_List_Select = "select p.id,p.pointId,p.name,a.id,p.type,p.sortid from appPoints p " +
	...
	class AppPointsRowMapper implements GenericRowMapper<AppPoints<?>> {
		public AppPoints<?> mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			point.setType(rs.getInt(5));
			point.setSortid(rs.getInt(6));
			return point;
		}
	}
	/**
	...
	public List<AppPoints<?>> getPointsByAcpId(String acpid) {
		String sql = Point_List_Select+" where p.acpId = ? order by p.sortid desc";
	...
	public boolean updatePoint(String pid, String name, String pointId, String aid, String pointType,String sortid) {
		//查询是否与其他点同名
		...
		String sql = "update appPoints set name = ?,pointId = ?,acpId = ?,type = ?,sortid = ? where id = ?";
		String update = "update appPoints set type = 0 where type = ? and acpId = ?";
		this.ejt.update(update,new Object[]{pointType,aid});
		this.ejt.update(sql, new Object[]{name,pointId,aid,pointType,sortid,pid});
		return true;
	}

	public boolean addPoint(String name, String pointId, String aid,String type,String sortid) {
		...
		if(count!=0)return false;
		String sql = "insert into appPoints(name,pointId,acpId,type,sortid) values(?,?,?,?,?)";
		this.ejt.update(sql, new Object[]{name,pointId,aid,type,sortid});
		return true;
	}
