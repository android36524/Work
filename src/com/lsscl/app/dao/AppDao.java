...
public abstract class AppDao extends BaseDao {
	 ...
	 * 单个空压机属性点
	 */
	protected static final String acp_attr2 = "select p.pointId,p.name,p.type from appAcps a "
			+ "left join appPoints p on a.id = p.acpId "
			+ "where a.id = ? order by p.sortid desc";
	/**
	 * 单个空压机属性点
	 */
	...
	protected int getBasePointId(String acpId, BasePoint power) {
		int type = power.getType();
		String sql = "select pointId from appPoints where acpId = ? and type = ?";
		int pid = queryForObject(sql, new Object[] { acpId, type },
				Integer.class, 0);
		return pid;
	}

	protected List<PointValueTime> getBasePoints(String acpId, BasePoint type,
			long from, long to) {
		int pid = getBasePointId(acpId, type);
		return pointValueDao.getPointValuesBetween(pid, from, to);
	}

	protected PointValueTime getBasePoint(String acpId, BasePoint type) {
		int pid = getBasePointId(acpId, type);
		return pointValueDao.getLatestPointValue(pid);
	}
	...
	protected boolean isNotAdmin(String phone, String enPwd) {
		...
		int count = queryForObject(sql, new Object[] { phone, enPwd },
				Integer.class, 0);
		return count == 1;
	}
	...
	protected boolean isNotAdmin(String phone) {
		...
		int count = queryForObject(sql, new Object[] { phone }, Integer.class,
				0);
		return count == 1;
	}
	...
	public boolean isRootScope(String phone, String scopeId) {
		...
		List<Map<String, String>> sids = ejt.query(sql, new Object[] { phone },
				new ResultDataMapper());
		if (sids.size() == 1) {
			Map<String, String> m = sids.get(0);
			return (!"3".equals(m.get("scopetype")))
					&& scopeId.equals(m.get("scopeid"));
		} else {
			return false;
		}
	}
	...
	protected int getPidByAcpType(String aid, int type) {
		String sql = "select pointId from appPoints where acpId = ? and type = ?";
		int pid = queryForObject(sql, new Object[] { aid, type },
				Integer.class, 0);
		return pid;
	}
	...
	protected String getUnitByPid(int pid) {
		...
		Integer type = 0;
		if (pvts.size() != 0)
			type = pvts.get(0).getValue().getDataType();
	 ...
	 * @return
	 */
	protected PointValueTime getPointValueByAcpIdAndName2(String aid,
			String attrName) {
		String getPid = "select pointId from appPoints where acpId = ? and name = ?";
		int pid = queryForObject(getPid, new Object[] { aid, attrName },
				Integer.class, 0);
	...
	protected boolean isRun(int id) {
		PointValueTime pvt = getPointValueByAcpIdAndName2(id + "",
				AcpConfig.RUN_STOP);
		if (pvt == null)
			return false;
		long now = System.currentTimeMillis();
		boolean flag = (now - pvt.getTime()) < (5 * 60 * 1000);
		if (!flag)
			return false;
		Double value = pvt.getDoubleValue();
		if (value != null) {
	 ...
	 * @return
	 */
	protected boolean isBasicPointName(Integer type) {
		if (type == null)
			return false;
		if (type == BasePoint.Current.getType()
				|| type == BasePoint.ExhaustPressure.getType()
				|| type == BasePoint.ExhaustTemperature.getType())
			return true;
		return false;
	}

	protected boolean isBasicPointName2(Integer type) {
		if (type == null)
			return false;
		return type <= 3;
	}
	...
	public void saveMobileEvent(Integer id) {
		// 查询点为报警点
		/*
		 * Map<String, Object> map = queryForObject(acp_attr3, new Object[] { id
		 * }, new com.serotonin.mango.vo.ResultData(), new HashMap<String,
		 * Object>()); Integer aid = (Integer) map.get("id"); String attrname =
		 * (String) map.get("attrname"); if (aid != null) { // Random rd = new
		 * Random(); int i = rd.nextInt(10); if (i > 5) {
		 * ejt.update("insert into pointValues_" + id +
		 * "(dataType,pointValue,ts)values(?,?,?)", new Object[] { 1, 1, new
		 * Date().getTime() }); } else { ejt.update("insert into pointValues_" +
		 * id + "(dataType,pointValue,ts)values(?,?,?)", new Object[] { 1, 0,
		 * new Date().getTime() }); }
		 * 
	...
	public void updateMobileEvent(Integer pid, Long time) {
		...
		/*
		 * if (time != null) {// 报警时间不为空 // 表不存在 if (!isPointExist(pid)) return;
		 * String sql = "select top 1 * from pointValues_" + pid +
		 * " order by id desc"; Map<String, Object> point = queryForObject(sql,
		 * null, new com.serotonin.mango.vo.ResultData(), new HashMap<String,
		 * Object>()); // 查询点信息 // 查询当前报警值 double data = (Double)
		 * point.get("pointValue"); if (data == 0) {// 无报警(将时间置为空)
	...
	private static final String getScopeAndLevelByActiveTs = "select DISTINCT e.scopeId,e.emailHandler,s.scopename from appEvents e "
			...
			+ "where s.scopetype=3 "
			+ "and e.ackTs is null " + "and e.activeTs > ?";
	...
	public DataPointVO getDataPointVO(int dpid) {
		...
		Map<String, Object> map = queryForObject(sql, new Object[] { dpid },
				new ResultData());
	...
	public void updateStatistics() {
		...
		List<Integer> sids = ejt.queryForList(sidSql, new Object[] {},
				Integer.class);
		for (int sid : sids) {
	...
	private void updateAcpStatisticsByType(int i) {
		...
		List<Map<String, Object>> data = ejt.query(sql, new Object[] { i },
				new ResultData());
		String insert = "insert into scopeStatistics (scopeId,power,openCount,closeCount)values(?,?,?,?)";
		for (Map<String, Object> m : data) {
			insertOrUpdateScopeStatistics(m.get("parentId"), m.get("power"),
					m.get("openCount"), m.get("closeCount"));
		}
	}

	private void updateAcpStatistics(int sid) {
		String sql = "select id from appAcps where scopeId = ?";
		List<Integer> aids = ejt.queryForList(sql, new Object[] { sid },
				Integer.class);
		double power = 0;
		int open = 0;
		for (int aid : aids) {
	...
	private void insertOrUpdateScopeStatistics(Object sid, Object power,
