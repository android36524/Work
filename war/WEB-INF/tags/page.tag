...
<meta name="DESCRIPTION"
	content="<fmt:message key='common.description'/>" />
...
<script type="text/javascript"
	src="resources/soundmanager2-nodebug-jsmin.js"></script>
...
<c:forEach items="${js}" var="jsname">
	<script type="text/javascript" src="resources/${jsname}.js"></script>
</c:forEach>
<script type="text/javascript">
...
<script type="text/javascript"
	src="/msdropdown/uncompressed.jquery.dd.js"></script>
...
<body>
			...
			<tr style="height: 48px">
				<c:if test="${!empty sessionUser}">
					<td rowspan="1" width="20%"
						style="padding: 0px 0px 0px 0px;background-color: #6ba53a;">
						<img src="${sessionUser.currentScope.backgroundFilename}"
						id="imgLogo"
						style="top:0px;left:0px;position:absolute;height: 65px;padding: 0px 0px 10px 0px;"
						alt="Logo" />
					</td>
					<td align="center" width="60%" style="background-color: #6ba53a;">
						<c:if test="${sessionUser.currentScope.scopetype==0}">
							<a href="center_events.shtm">
						</c:if> <c:if test="${sessionUser.currentScope.scopetype==1}">
							<a href="zone_events.shtm?zoneId=${sessionUser.currentScope.id}">
						</c:if> <c:if test="${sessionUser.currentScope.scopetype==2}">
							<a
								href="subzone_events.shtm?subzoneId=${sessionUser.currentScope.id}">
						</c:if> <c:if test="${sessionUser.currentScope.scopetype==3}">
							<a
								href="factory_events.shtm?factoryId=${sessionUser.currentScope.id}">
						</c:if> <span id="__header__alarmLevelDiv" style="display:none;">
							<img id="__header__alarmLevelImg" src="images/spacer.gif" alt=""
							border="0" title="" /> <span style="color:#FFFFFF;"
							id="__header__alarmLevelText"></span>
					</span> </a>
					</td>
				</c:if>
				<c:if test="${!empty sessionUser}">
					<td align="right" width="20%" class="copyTitle"
						style=" white-space: nowrap; background-color: #6ba53a;"><span><fmt:message
								key="header.user" />: <b>${sessionUser.username}</b></span> <span><fmt:message
								key="header.scope.currentScop" />: <b>${sessionUser.currentScope.scopename}</b></span>
						<tag:img id="userMutedImg"
							onclick="MiscDwr.toggleUserMuted(setUserMuted)"
							onmouseover="hideLayer('localeEdit')" /> <tag:img png="house"
							title="header.goHomeUrl" onclick="goHomeUrl()"
							onmouseover="hideLayer('localeEdit')" /> <tag:img
							png="house_link" title="header.setHomeUrl" onclick="setHomeUrl()"
							onmouseover="hideLayer('localeEdit')" />
						<div style="display:inline;"
							onmouseover="showMenu('localeEdit', -40, 10);">
							<tag:img png="world" title="header.changeLanguage" />
							<div id="localeEdit" style="visibility:hidden;left:0px;top:15px;"
								class="labelDiv" onmouseout="hideLayer(this)">
								<c:forEach items="${availableLanguages}" var="lang">
									<a class="ptr" onclick="setLocale('${lang.key}')">${lang.value}</a>
									<br />
								</c:forEach>
							</div>
						</div></td>
    ...
    -->
			...
			<tr style="height: 25px;">
				<td rowspan="1" width="20%" style="padding: 0px 0px 0px 0px;">
					<img src="logos/4_hid.png" id="imgLogohid"
					style="padding: 0px 0px 10px 0px;" />
				</td>
				<c:if test="${!simple}">
					<td style="cursor:default;" align="center"><c:if
							test="${!empty sessionUser}">
							<c:if test="${sessionUser.currentScope.scopetype==0}">
								<tag:menuItem href="zone_list.shtm" png="scope/zoneList"
									key="header.zoneList" />
								<tag:menuItem href="center_factory_search.shtm"
									png="scope/factorySearch" key="header.factorySearch" />
          ...
          <tag:menuItem href="center_statisticsIndexOrder.shtm" png="statisticsIndexOrder" key="header.statisticsIndexOrder"/>
           -->
								<tag:menuItem href="center_users.shtm" png="userpng"
									key="header.centerUser" />

							</c:if>
							<c:if test="${sessionUser.currentScope.scopetype==1}">
								<tag:menuItem
									href="subzone_list.shtm?zoneId=${sessionUser.currentScope.id}"
									png="scope/subZone" key="header.subZoneList" />
								<tag:menuItem
									href="zone_factory_search.shtm?zoneId=${sessionUser.currentScope.id}"
									png="scope/factorySearch" key="header.factorySearch" />
         ...
         -->
								<c:if
									test="${sessionUser.admin||sessionUser.tempAdmin||sessionUser.homeScope.scopetype==1}">
									<tag:menuItem
										href="zone_users.shtm?zoneId=${sessionUser.currentScope.id}"
										png="userpng" key="header.zoneUser" />
								</c:if>
							</c:if>
							<c:if test="${sessionUser.currentScope.scopetype==2}">
								<tag:menuItem
									href="factory_list.shtm?subzoneId=${sessionUser.currentScope.id}"
									png="scope/factorySearchnew" key="header.factoryList" />
								<tag:menuItem
									href="subzone_factory_search.shtm?subzoneId=${sessionUser.currentScope.id}"
									png="scope/factorySearch" key="header.factorySearch" />
          ...
          <tag:menuItem href="subzone_statisticsIndexOrder.shtm?subzoneId=${sessionUser.currentScope.id}" png="statisticsIndexOrder" key="header.statisticsIndexOrder"/>
       	 	 -->
								<c:if
									test="${sessionUser.admin||sessionUser.tempAdmin||sessionUser.homeScope.scopetype==2}">
									<tag:menuItem
										href="subzone_users.shtm?subzoneId=${sessionUser.currentScope.id}"
										png="userpng" key="header.users" />
								</c:if>
							</c:if>
							<c:if test="${sessionUser.currentScope.scopetype==3}">
								<tag:menuItem
									href="watch_list.shtm?factoryId=${sessionUser.currentScope.id}"
									png="eye" key="header.watchlist" />
								<!--<tag:menuItem href="dynamicData.shtm?factoryId=${sessionUser.currentScope.id}" png="action_log" key="header.realtimeData" />-->
								<tag:menuItem
									href="views.shtm?factoryId=${sessionUser.currentScope.id}"
									png="icon_viewnew" key="header.views" />
								<!--<tag:menuItem href="lssclMap.shtm?factoryId=${sessionUser.currentScope.id}" png="google_ico/earth" key="header.factoryList" />-->
								<c:if
									test="${sessionUser.admin||sessionUser.tempAdmin||sessionUser.homeScope.scopetype==3}">
									<tag:menuItem
										href="factory_users.shtm?factoryId=${sessionUser.currentScope.id}"
										png="userpng" key="header.users" />
								</c:if>
								<tag:menuItem
									href="factory_events.shtm?factoryId=${sessionUser.currentScope.id}"
									png="flag_whitenew" key="header.alarms" />
								<c:if test="${sessionUser.defaultRole.id==1}">
									<tag:menuItem
										href="reports.shtm?factoryId=${sessionUser.currentScope.id}"
										png="reportnew" key="header.reports" />
								</c:if>
							</c:if>
							<c:if
								test="${sessionUser.dataSourcePermission||sessionUser.tempAdmin}">
								<c:if test="${sessionUser.currentScope.scopetype==3}">
									<c:if
										test="${sessionUser.defaultRole.id==1||sessionUser.defaultRole.id==3||sessionUser.defaultRole.id==5||sessionUser.tempAdmin}">
										<tag:menuItem
											href="data_sources.shtm?factoryId=${sessionUser.currentScope.id}"
											png="icon_dsnew" key="header.dataSources" />
										<tag:menuItem
											href="scheduled_events.shtm?factoryId=${sessionUser.currentScope.id}"
											png="clocknew" key="header.scheduledEvents" />
										<tag:menuItem
											href="compound_events.shtm?factoryId=${sessionUser.currentScope.id}"
											png="multi_bellnew" key="header.compoundEvents" />
										<tag:menuItem
											href="point_links.shtm?factoryId=${sessionUser.currentScope.id}"
											png="linknew" key="header.pointLinks" />
									</c:if>
								</c:if>
							</c:if>
							...
							<c:if test="${sessionUser.admin||sessionUser.tempAdmin}">
								<c:if test="${sessionUser.currentScope.scopetype==0}">
									<tag:menuItem href="statistics_config.shtm"
										png="scope/statistics" key="header.statistics" />
									<tag:menuItem href="center_system_settings.shtm"
										png="scope/systemnew" key="header.centerSystemSetting" />
									<tag:menuItem href="zone_edit.shtm" png="scope/edit_diff"
										key="header.zoneEdit" />
									<tag:menuItem href="acp_type_attr.shtm" png="scope/acpAttrnew"
										key="header.acpAttr" />
								</c:if>
								<c:if test="${sessionUser.currentScope.scopetype==1}">
									<tag:menuItem
										href="zone_system_settings.shtm?zoneId=${sessionUser.currentScope.id}"
										png="scope/system" key="header.zoneSystemSetting" />
									<tag:menuItem
										href="subzone_edit.shtm?zoneId=${sessionUser.currentScope.id}"
										png="scope/edit_diff" key="header.subZoneEdit" />
								</c:if>
								<c:if test="${sessionUser.currentScope.scopetype==2}">
									<tag:menuItem
										href="subzone_system_settings.shtm?subzoneId=${sessionUser.currentScope.id}"
										png="scope/system" key="header.zoneSystemSetting" />
									<tag:menuItem
										href="factory_edit.shtm?subzoneId=${sessionUser.currentScope.id}"
										png="scope/factoryedit" key="header.factoryEdit" />
								</c:if>
								<c:if test="${sessionUser.currentScope.scopetype==3}">
									<tag:menuItem
										href="event_handlers.shtm?factoryId=${sessionUser.currentScope.id}"
										png="cognew" key="header.eventHandlers" />
									<c:if
										test="${sessionUser.defaultRole.id==1||sessionUser.defaultRole.id==3||sessionUser.defaultRole.id==5}">
										<tag:menuItem
											href="compress_air_system.shtm?factoryId=${sessionUser.currentScope.id}"
											png="acpSettingnew" key="header.acpSetting" />
										<tag:menuItem
											href="point_hierarchy.shtm?factoryId=${sessionUser.currentScope.id}"
											png="folder_bricknew" key="header.pointHierarchy" />
									</c:if>
									<tag:menuItem
										href="mailing_lists.shtm?factoryId=${sessionUser.currentScope.id}"
										png="booknew" key="header.mailingLists" />
									<c:if test="false">
										<tag:menuItem href="publishers.shtm" png="transmit"
											key="header.publishers" />
									</c:if>
									<!-- <tag:menuItem href="maintenance_events.shtm" png="hammer" key="header.maintenanceEvents"/> -->
									<tag:menuItem
										href="factory_system_settings.shtm?factoryId=${sessionUser.currentScope.id}"
										png="scope/system" key="header.factory_setting" />
									<c:if
										test="${sessionUser.defaultRole.id==1||sessionUser.defaultRole.id==2||sessionUser.defaultRole.id==3||sessionUser.defaultRole.id==5}">
										<tag:menuItem
											href="appsetting.shtm?factoryId=${sessionUser.currentScope.id}"
											png="scope/systemnew" key="header.factory_appSetting" />
									</c:if>

									<c:if
										test="${sessionUser.defaultRole.id==1||sessionUser.defaultRole.id==3||sessionUser.defaultRole.id==5}">
										<tag:menuItem href="remoteControl.shtm?factoryId=${sessionUser.currentScope.id}" png="slavecmd"
											key="header.remotecontrol" />
									</c:if>
								</c:if>
							</c:if>
							<tag:menuItem href="logout.htm" png="control_stop_blue"
								key="header.logout" />
						</c:if> <!--  <c:if test="${empty sessionUser}">
          <tag:menuItem href="login.htm" png="control_play_blue" key="header.login"/>
        </c:if>
         -->
						<div id="headerMenuDescription" class="labelDiv"
							style="position:absolute;display:none;"></div></td>
					<td align="right" style="padding:5px; white-space: nowrap;"><c:if
							test="${!empty sessionUser}">
							<input type="hidden" id="header_zoneId"
								value="<c:if test="${sessionUser.homeScope.scopetype==1}">${sessionUser.homeScope.id}</c:if><c:if test="${sessionUser.homeScope.scopetype!=1}" >-1</c:if>" />
							<input type="hidden" id="header_subzoneId"
								value="<c:if test="${sessionUser.homeScope.scopetype==2}">${sessionUser.homeScope.id}</c:if><c:if test="${sessionUser.homeScope.scopetype!=2}" >-1</c:if>" />
							<table>
								<tr>
									<td><span style="font-size: 11px;color: green;"
										id="myManagerHome" onmouseover="changeCursorStyle(this)"
										onclick="scopeSelector(event,0)"><fmt:message
												key="header.scope.mymanagerhome" /></span></td>
									<td>
										<!-- <c:if test="${sessionUser.homeScope.scopetype==0}"> --> <select
										id="zoneManagerHome" name="zoneManagerHome" tabindex="2"
										style="display:none"></select> <!-- <span style="font-size: 11px;color: green;"  id="zoneManagerHome"  onmouseover="changeCursorStyle(this)" onclick="scopeSelector(event,1)" ><fmt:message key="header.scope.zonemanagerhome"/></span>&nbsp;&nbsp;&nbsp;&nbsp; -->
										<!-- </c:if> -->
									</td>
									<td>
										<!-- <c:if test="${sessionUser.homeScope.scopetype<=1}"> --> <select
										id="subzoneManagerHome" name="subzoneManagerHome" tabindex="3"
										style="display:none"></select> <!-- <span style="font-size: 11px;color: green;"  id="subzoneManagerHome"  onmouseover="changeCursorStyle(this)" onclick="scopeSelector(event,2)" ><fmt:message key="header.scope.subzonemanagerhome"/></span>&nbsp;&nbsp;&nbsp;&nbsp; -->
										<!-- </c:if> -->
									</td>
									<td>
										<!-- <c:if test="${sessionUser.homeScope.scopetype<=2}"> --> <select
										id="factoryManagerHome" name="factoryManagerHome" tabindex="4"
										style="display:none"></select> <!--<span style="font-size: 11px;color: green;"  id="factoryManagerHome"  onmouseover="changeCursorStyle(this)" onclick="scopeSelector(event,3)" ><fmt:message key="herder.scope.factorymanagerhome"/></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; -->
										<!-- </c:if> -->
									</td>
								</tr>
							</table>
	...
	<div id="selector"
		style="font-size: 11px;color:#FFFFFF;width:150px;height:10px;overflow-x:auto;overflow-y:auto;cursor:pointer;z-index=10;">
		<table>
