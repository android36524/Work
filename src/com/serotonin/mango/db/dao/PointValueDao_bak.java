...
public class PointValueDao_bak extends BaseDao {
	...
	public PointValueDao_bak() {
		super();
	}

	/*
	 * static{ try { MongoOptions options = new MongoOptions();
	 * options.connectionsPerHost = 9000000; options.maxWaitTime = 1000000;
	 * options.socketKeepAlive = true;
	 * options.threadsAllowedToBlockForConnectionMultiplier = 1000;
	 * options.connectionsPerHost = 30000; options.maxWaitTime = 200000;
	 * options.socketKeepAlive = true;
	 * options.threadsAllowedToBlockForConnectionMultiplier = 200; Mongo mongo =
	 * new Mongo("127.0.0.1",options); mongodb = mongo.getDB("test");
	 * 
	...
	public PointValueTime savePointValueSync(int pointId,
		...
		PointValueTime savedPointValue = null;
		/*
		 * int retries = 5; while (true) { try { savedPointValue =
		 * getPointValue(pointId,id); break; } catch
		 * (ConcurrencyFailureException e) { if (retries <= 0) throw e;
		 * retries--; } }
		 */
	...
	long savePointValueImpl(final int pointId, final PointValueTime pointValue,
		...
		// final PointValueTime pointValue=new
		// PointValueTime(pointValue1.getValue(),ca.getTimeInMillis());
		...
		try {
			if (svalue != null || source != null || dataType == DataTypes.IMAGE) {
				...
				/*
				 * id = getTransactionTemplate().execute( new
				 * GenericTransactionCallback<Long>() { public Long
				 * doInTransaction(TransactionStatus status) { return
				 * savePointValue(pointId, dataType, dvalueFinal,
				 * pointValue.getTime(), svalueFinal, source, false); } });
				 */
				id = savePointValue(pointId, dataType, dvalueFinal,
						pointValue.getTime(), svalueFinal, source, false);
			} else
	...
	private long savePointValueImpl(int pointId, int dataType, double dvalue,
		 ...
		 */
		String POINT_VALUE_INSERT_SQL = "insert into pointvalues_" + pointId
				+ " (dataType, pointValue, ts) select ?,?,? ";
		// FIXME id? long id= doInsertLong(POINT_VALUE_INSERT_SQL, new Object[]
		// {dataType, dvalue, time });
		long id = 1;
		insertPoint(pointId, dataType, dvalue, time,svalue);
		/*
		 * if (svalue == null && dataType == DataTypes.IMAGE) svalue =
		 * Long.toString(id);
		 * 
		 * // Check if we need to create an annotation. if (svalue != null ||
		 * source != null) { Integer sourceType = null, sourceId = null; if
		 * (source != null) { sourceType = source.getSetPointSourceType();
		 * sourceId = source.getSetPointSourceId(); }
		 * 
		 * String shortString = null; String longString = null; if (svalue !=
		 * null) { if (svalue.length() > 128) longString = svalue; else
		 * shortString = svalue; }
		 * ----------------------------------------------
		 * ------------------------ ejt.update(POINT_VALUE_ANNOTATION_INSERT,
		 * new Object[] { id,pointId, shortString, longString, sourceType,
		 * sourceId }, new int[] { Types.INTEGER,Types.INTEGER, Types.VARCHAR,
		 * Types.CLOB, Types.SMALLINT, Types.INTEGER }); }
		 */
	...
	public List<PointValueTime> getPointValues(int dataPointId, long since) {
		...
		gt.put("$gt", since);
		query.put("t", gt);
		query.put("i", dataPointId);
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (dataPointId % POINT_COUNT));
		List<PointValueTime> pvts = new ArrayList<PointValueTime>();

		DBCursor cursor = collection.find(query).sort(
				new BasicDBObject("t", 1));
		while (cursor.hasNext()) {
			DBObject obj = cursor.next();
			MangoValue value = createMangoValue(obj);
			pvts.add(new PointValueTime(value, (Long) obj.get("t")));
		}
		cursor.close();
		return pvts;
	}
	...
	public PointValueTime getPointValuesForWarnCode(int dataPointId, long time) {
		DBObject query = new BasicDBObject();
		query.put("t", new BasicDBObject("$lte", time));
		query.put("i", dataPointId);
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (dataPointId % POINT_COUNT));
		...
		if (cursor.hasNext()) {
			DBObject obj = cursor.next();
			MangoValue value = createMangoValue(obj);
			return new PointValueTime(value, (Long) obj.get("t"));
		}
		cursor.close();
		return null;
	}
	...
	public List<PointValueTime> getPointValuesBetween(int dataPointId,
		 ...
		 * + "  pva.sourceId " + "from pointValues_"+dataPointId+" pv " +
		 * "  left join pointValueAnnotations pva on pv.id = pva.pointValueId and pva.pointId="
		 * +dataPointId + " where  pv.ts >= ? and pv.ts<? order by ts", new
		 * Object[] { from, to }, 0);
		 */
		DBObject query = new BasicDBObject();
		...
		condition.put("$lt", to);
		query.put("i", dataPointId);
		query.put("t", condition);
		
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (dataPointId % POINT_COUNT));
		List<PointValueTime> pvts = new ArrayList<PointValueTime>();
		DBCursor cursor = collection.find(query).limit(4320);
		while (cursor.hasNext()) {
			DBObject obj = cursor.next();
			MangoValue value = createMangoValue(obj);
			pvts.add(new PointValueTime(value, (Long) obj.get("t")));
	...
	public List<DBObject> getPointValuesBetween2(int dataPointId,
			long from, long to) {
		...
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (dataPointId % POINT_COUNT));
		DBCursor cursor = collection.find(query).batchSize(4320);
		System.out.println("time:"+(System.currentTimeMillis()-l));
		List<DBObject> list = cursor.toArray();
		System.out.println("t2:"+(System.currentTimeMillis()-l));
		return list;
	}
	
	public List<DBObject> getPointDBObjectsBetween(int dataPointId,
			long from, long to) {
		/*
		 * return pointValuesQuery(
		 * "select pv.dataType, pv.pointValue, pva.textPointValueShort, pva.textPointValueLong, pv.ts, pva.sourceType, "
	...
	public List<PointValueTime> getLatestPointValues(int dataPointId, int limit) {
		 ...
		 * + "  pva.sourceId " + "from pointValues_"+dataPointId+" pv " +
		 * "  left join pointValueAnnotations pva on pv.id = pva.pointValueId and pva.pointId="
		 * +dataPointId + "  order by pv.ts desc", new Object[] { }, limit);
		 */
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (dataPointId % POINT_COUNT));
		List<PointValueTime> pvts = new ArrayList<PointValueTime>();
		DBCursor cursor = collection
				.find(new BasicDBObject("i", dataPointId))
				.sort(new BasicDBObject("t", -1)).limit(limit);
		while (cursor.hasNext()) {
			DBObject obj = cursor.next();
			MangoValue value = createMangoValue(obj);
			pvts.add(new PointValueTime(value, (Long) obj.get("t")));
	 ...
	 * @return
	 */
	public List<DBObject> getLatestDBObjects(int dataPointId, int limit) {
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (dataPointId % POINT_COUNT));
		List<DBObject> pvts = new ArrayList<DBObject>();
		DBCursor cursor = collection
	...
	public List<PointValueTime> getLatestPointValues(int dataPointId,
		 ...
		 * + "  pva.sourceId " + "from pointValues_"+dataPointId+" pv " +
		 * "  left join pointValueAnnotations pva on pv.id = pva.pointValueId  and pva.pointId="
		 * +dataPointId + " where  pv.ts<? order by pv.ts desc", new Object[] {
		 * before }, limit);
		 */
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (dataPointId % POINT_COUNT));
		DBObject query = new BasicDBObject();
		query.put("ts", new BasicDBObject("lt", before));
		query.put("i", dataPointId);
		List<PointValueTime> pvts = new ArrayList<PointValueTime>();
		DBCursor cursor = collection.find(query)
				.sort(new BasicDBObject("t", -1)).limit(limit);
	...
	public PointValueTime getLatestPointValue(int dataPointId) {
		 ...
		 * "select max(ts) from pointValues_"+dataPointId, new Object[] {},0);
		 * if (maxTs == 0) return null; return pointValueQuery(
		 * "select pv.dataType, pv.pointValue, pva.textPointValueShort, pva.textPointValueLong, pv.ts, pva.sourceType, "
		 * + "  pva.sourceId " + "from pointValues_"+dataPointId+" pv " +
		 * "  left join pointValueAnnotations pva on pv.id = pva.pointValueId  and pva.pointId="
		 * +dataPointId + " where pv.ts=?", new Object[] { maxTs });
		 */
		DBObject query = new BasicDBObject();
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (dataPointId % POINT_COUNT));
	 ...
	 * @return
	 */
	public DBObject getLatestPointValueDBObject(int dataPointId) {
		/*
		 * long maxTs = ejt.queryForLong(
		 * "select max(ts) from pointValues_"+dataPointId, new Object[] {},0);
		 * if (maxTs == 0) return null; return pointValueQuery(
	...
	// ok
	/*
	 * private PointValueTime getPointValue(int pointId,long id) { return
	 * pointValueQuery(
	 * "select pv.dataType, pv.pointValue, pva.textPointValueShort, pva.textPointValueLong, pv.ts, pva.sourceType, "
	 * + "  pva.sourceId " + "from pointValues_"+pointId+" pv " +
	 * "left join pointValueAnnotations pva on pv.id = pva.pointValueId  and pva.pointId="
	 * +pointId + " where pv.id=?", new Object[] { id }); }
	 */
	// ok
	public PointValueTime getPointValueBefore(int dataPointId, long time) {
		/*
		 * Long valueTime = queryForObject(
		 * "select max(ts) from pointValues_"+dataPointId+" where ts<?", new
		 * Object[] { time }, Long.class, null); if (valueTime == null) return
		 * null; return getPointValueAt(dataPointId, valueTime);
		 */
		DBObject query = new BasicDBObject();
		query.put("ts", new BasicDBObject("lt", time));
		query.put("i", dataPointId);
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (dataPointId % POINT_COUNT));
		DBCursor cursor = collection.find(query)
	...
	public PointValueTime getPointValueAt(int dataPointId, long time) {
		 ...
		 * + "  pva.sourceId " + "from pointValues_"+dataPointId+" pv " +
		 * "left join pointValueAnnotations pva on pv.id = pva.pointValueId  and pva.pointId="
		 * +dataPointId + " where pv.ts=?", new Object[] {time });
		 */
		DBObject query = new BasicDBObject();
		query.put("t", time);
		query.put("i", dataPointId);
	 ...
	 * List<PointValueTime> result = pointValuesQuery(sql, params, 1); if
	 * (result.size() == 0) return null; return result.get(0); }
	 */
	//
	/*
	 * private List<PointValueTime> pointValuesQuery(String sql, Object[]
	 * params, int limit) { List<PointValueTime> result = query(sql, params, new
	 * PointValueRowMapper(), limit); updateAnnotations(result); return result;
	 * }
	 */
	// ok
	public void getPointValuesBetween(int dataPointId, long from, long to,
		 ...
		 * "left join pointValueAnnotations pva on pv.id = pva.pointValueId  and pva.pointId="
		 * +dataPointId + " where pv.ts >= ? and pv.ts<?  order by ts", new
		 * Object[] {from, to }, new PointValueRowMapper(), callback);
		 */
		List<IdPointValueTime> idPointValueTimes = new ArrayList<IdPointValueTime>();

		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (dataPointId % POINT_COUNT));
		DBObject tsQuery = new BasicDBObject();
		tsQuery.put("gte", from);
		tsQuery.put("lt", to);
		DBObject query = new BasicDBObject();
		query.put("i", dataPointId);
		query.put("t", tsQuery);
		...
		while (cursor.hasNext()) {
			DBObject obj = cursor.next();
			MangoValue value = createMangoValue(obj);
			idPointValueTimes.add(new IdPointValueTime(dataPointId, value,
					(Long) obj.get("t")));
		}
		cursor.close();
		Collections.sort(idPointValueTimes);
		for (int i = 0; i < idPointValueTimes.size(); i++) {
			IdPointValueTime idPointValueTime = idPointValueTimes.get(i);
			callback.row(idPointValueTime, i);
		}
	}

	/*
	 * class PointValueRowMapper implements GenericRowMapper<PointValueTime> {
	 * public PointValueTime mapRow(ResultSet rs, int rowNum) throws
	 * SQLException { MangoValue value = createMangoValue(rs, 1); long time =
	 * rs.getLong(5);
	 * 
	 * int sourceType = rs.getInt(6); if (rs.wasNull()) // No annotations, just
	 * return a point value. return new PointValueTime(value, time);
	 * 
	 * // There was a source for the point value, so return an annotated //
	 * version. return new AnnotatedPointValueTime(value, time, sourceType, rs
	 * .getInt(7)); } }
	 */
	...
	MangoValue createMangoValue(DBObject rs) {
		int dataType = (Integer) rs.get("e");
		Object obj = rs.get("v");
		
		MangoValue value = MangoValue.objectToValue(obj);;
		switch (dataType) {
		case (DataTypes.NUMERIC):
			value = new NumericValue((Double) obj);
			break;
		case (DataTypes.BINARY):
			value = new BinaryValue((Double) obj == 1);
			break;
		case (DataTypes.MULTISTATE):
			double d = (Double) obj;
			value = new MultistateValue((int) d);
			break;
		case (DataTypes.ALPHANUMERIC):
			value = new AlphanumericValue(obj.toString());
		/*
		 * case (DataTypes.ALPHANUMERIC): String s = rs.getString(firstParameter
		 * + 2); if (s == null) s = rs.getString(firstParameter + 3); value =
		 * new AlphanumericValue(s); break; case (DataTypes.IMAGE): value = new
		 * ImageValue(Integer.parseInt(rs .getString(firstParameter + 2)), rs
		 * .getInt(firstParameter + 3)); break;
		 */
		default:
			value = null;
		}
	 ...
	 * Map<Integer, List<AnnotatedPointValueTime>> userIds = new
	 * HashMap<Integer, List<AnnotatedPointValueTime>>();
	 * List<AnnotatedPointValueTime> alist;
	 * 
	 * // Look for annotated point values. AnnotatedPointValueTime apv; for
	 * (PointValueTime pv : values) { if (pv instanceof AnnotatedPointValueTime)
	 * { apv = (AnnotatedPointValueTime) pv; if (apv.getSourceType() ==
	 * SetPointSource.Types.USER) { alist = userIds.get(apv.getSourceId()); if
	 * (alist == null) { alist = new ArrayList<AnnotatedPointValueTime>();
	 * userIds.put(apv.getSourceId(), alist); } alist.add(apv); } } }
	 * 
	 * // Get the usernames from the database. if (userIds.size() > 0)
	 * updateAnnotations("select id, username from users where id in ",
	 * userIds); }
	 */

	/*
	 * private void updateAnnotations(String sql, Map<Integer,
	 * List<AnnotatedPointValueTime>> idMap) { // Get the description
	 * information from the database. List<IntValuePair> data = query(sql + "("
	 * + createDelimitedList(idMap.keySet(), ",", null) + ")", new
	 * GenericIntValuePairRowMapper());
	 * 
	 * // Collate the data with the id map, and set the values in the //
	 * annotations List<AnnotatedPointValueTime> annos; for (IntValuePair ivp :
	 * data) { annos = idMap.get(ivp.getKey()); for (AnnotatedPointValueTime avp
	 * : annos) avp.setSourceDescriptionArgument(ivp.getValue()); } }
	 */
	...
	public void getPointValuesBetween(List<Integer> dataPointIds, long from,
		...
		// String ids = createDelimitedList(dataPointIds, ",", null);
		/*
		 * String sql=""; for (int i = 0; i < dataPointIds.size(); i++) { int
		 * id=dataPointIds.get(i);
		 * sql+="select "+id+" as dataPointId, pv_"+id+".dataType, pv_"
		 * +id+".pointValue, " // +
		 * "pva_"+id+".textPointValueShort, pva_"+id+".textPointValueLong, pv_"
		 * +id+".ts " + "from pointValues_"+id +
		 * " as "+"pv_"+id+" left join pointValueAnnotations pva_"
		 * +id+" on pv_"+id
		 * +".id = pva_"+id+".pointValueId and pva_"+id+".pointId ="+id
		 * +" where pv_"+id+".ts >= "+from+" and pv_"+id+".ts<"+to+" ";
		 * if(i<dataPointIds.size()-1){ sql+=" union all "; } } query(sql +
		 * " order by ts", new Object[] { }, new IdPointValueRowMapper(),
		 * callback);
		 */
		List<IdPointValueTime> idPointValueTimes = new ArrayList<IdPointValueTime>();

		for (int i = 0; i < dataPointIds.size(); i++) {
			int pid = dataPointIds.get(i);
			DBCollection collection = MongoUtil.getColl("pointValues_"
					+ (pid % POINT_COUNT));
			DBObject tsQuery = new BasicDBObject();
			tsQuery.put("gte", from);
			...
			while (cursor.hasNext()) {
				DBObject obj = cursor.next();
				MangoValue value = createMangoValue(obj);
				idPointValueTimes.add(new IdPointValueTime(pid, value,
						(Long) obj.get("t")));
			}
			cursor.close();
		}
	...
	public long deletePointValuesBefore(int dataPointId, long time) {
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (dataPointId % POINT_COUNT));
		DBObject query = new BasicDBObject("t", new BasicDBObject("lt", time));
		query.put("i", dataPointId);
		WriteResult wr = collection.remove(query);
		return wr.getN();
		/*
		 * return deletePointValues(
		 * "delete from pointValues_"+dataPointId+" where  ts<?", new Object[] {
		 * time });
		 */
	}
	...
	private long deletePointValuesByTime(int dataPointId, long time, long time2) {
		...
		DBObject query = new BasicDBObject();
		DBObject condition = new BasicDBObject();
		condition.put("$gte", time);
		condition.put("$lt", time2);
		query.put("t", condition);
		query.put("i", dataPointId);
		WriteResult wr = collection.remove(query);
		return wr.getN();
	...
	public void deletePointValuesByTime(List<Integer> dataPointIds, long time,
			long time2) {
		for (int i = 0; i < dataPointIds.size(); i++) {
			/*
			 * deletePointValues(
			 * "delete from pointValues_"+dataPointIds.get(i)+
			 * " where ts between ? and ? ", new Object[] { time,time2});
			 */
			deletePointValuesByTime(dataPointIds.get(i), time, time2);
		}
	}
	...
	public long deletePointValues(int dataPointId) {
		/*
		 * return deletePointValues("delete from pointValues_"+dataPointId, new
		 * Object[] {});
		 */
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (dataPointId % POINT_COUNT));
	...
	public long deletePointValuesWithMismatchedType(int dataPointId,
		 ...
		 * +" where dataType<>?", new Object[] {dataType });
		 */
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (dataPointId % POINT_COUNT));
		DBObject query = new BasicDBObject();
		DBObject condition = new BasicDBObject();
		condition.put("$ne", dataType);
		query.put("e", condition);
		query.put("i", dataPointId);
		WriteResult wr = collection.remove(query);
		return wr.getN();
	}
	...
	/*
	 * private long deletePointValues(String sql, Object[] params) { int cnt; if
	 * (params == null) cnt = ejt.update(sql); else cnt = ejt.update(sql,
	 * params); clearUnsavedPointValues(); return cnt; }
	 */
	// ok
	public long dateRangeCount(int dataPointId, long from, long to) {
		/*
		 * return ejt .queryForLong(
		 * "select count(*) from pointValues_"+dataPointId
		 * +" where ts>=? and ts<=?", new Object[] { from, to });
		 */
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (dataPointId % POINT_COUNT));
		DBObject query = new BasicDBObject();
	...
	public long getInceptionDate(int dataPointId) {
		/*
		 * return ejt.queryForLong(
		 * "select min(ts) from pointValues_"+dataPointId, new Object[] {}, -1);
		 */
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (dataPointId % POINT_COUNT));
		DBObject ts = new BasicDBObject("t", 1);
		DBObject obj = collection.findOne(
				new BasicDBObject("i", dataPointId), null, ts);
		if (obj == null)
			return -1;
		return (Long) obj.get("t");
	}
	...
	public long getStartTime(List<Integer> dataPointIds) {
		if (dataPointIds.isEmpty())
			return -1;
		/*
		 * String sql="select min(ts)as ts from ("; for (int i = 0; i <
		 * dataPointIds.size(); i++) {
		 * sql+="select min(ts) as ts from pointValues_"+dataPointIds.get(i);
		 * if(i<dataPointIds.size()-1) sql+=" union all "; } return ejt
		...
		for (int i = 0; i < dataPointIds.size(); i++) {
			...
			DBCollection collection = MongoUtil.getColl("pointValues_"
					+ (pid % POINT_COUNT));
			DBObject ts = new BasicDBObject("t", 1);
			DBObject obj = collection.findOne(new BasicDBObject("i", pid),
					null, ts);
			if (obj != null) {
				long time = (Long) obj.get("t");
				l = time <= l ? time : l;
			}
		}
		return l;
	...
	public long getEndTime(List<Integer> dataPointIds) {
		if (dataPointIds.isEmpty())
			return -1;
		/*
		 * String sql="select max(ts)as ts from ("; for (int i = 0; i <
		 * dataPointIds.size(); i++) {
	...
	public boolean getEndDataTime(List<Integer> dataPointIds, long time) {
		/*
		 * if (dataPointIds.isEmpty()) return false; String
		 * sql="select max(ts)as ts from ("; for (int i = 0; i <
		 * dataPointIds.size(); i++) {
		 * sql+="select max(ts) as ts from pointValues_"
		 * +dataPointIds.get(i)+" where ts between "
		 * +(time-20000)+" and "+(time+20000); if(i<dataPointIds.size()-1)
		 * sql+=" union all "; } long ts= ejt .queryForLong(sql+") as tsTable");
		 * if(ts!=0.0){ return true; }
		 */
		long l = -1;
		for (int i = 0; i < dataPointIds.size(); i++) {
			int pid = dataPointIds.get(i);
			DBCollection collection = MongoUtil.getColl("pointValues_"
					+ (pid % POINT_COUNT));
			DBObject ts = new BasicDBObject("t", -1);
			DBObject between = new BasicDBObject();
			between.put("$gte", time - 20000);
			between.put("$lte", time + 20000);
			DBObject query = new BasicDBObject();
			query.put("t", between);
			query.put("i", pid);
			DBObject obj = collection.findOne(query, null, ts);
			if (obj != null) {
				long ll = (Long) obj.get("t");
				l = ll > l ? ll : l;
				break;
			}
		}
	...
	public long getEndDataTime(List<Integer> dataPointIds) {
		/*
		 * if (dataPointIds.isEmpty()) return 0; String
		 * sql="select max(ts)as ts from ("; for (int i = 0; i <
		 * dataPointIds.size(); i++) {
		 * sql+="select max(ts) as ts from pointValues_"+dataPointIds.get(i);
		 * if(i<dataPointIds.size()-1) sql+=" union all "; } return ejt
		 * .queryForLong(sql+") as tsTable");
		 */
		return getEndTime(dataPointIds);
	}

	// ok
	public LongPair getStartAndEndTime(List<Integer> dataPointIds) {
		/*
		 * if (dataPointIds.isEmpty()) return null; String
		 * sql="select min(min) as min,max(max)as max from ("; for (int i = 0; i
		 * < dataPointIds.size(); i++) {
		 * sql+="select min(ts) as min,max(ts) as max from pointValues_"
		 * +dataPointIds.get(i); if(i<dataPointIds.size()-1) sql+=" union all ";
		 * }
		 * 
		 * return queryForObject(sql+") as tableTs", null, new
		 * GenericRowMapper<LongPair>() {
		 * 
		 * @Override public LongPair mapRow(ResultSet rs, int index) throws
		 * SQLException { long l = rs.getLong(1); if (rs.wasNull()) return null;
		 * return new LongPair(l, rs.getLong(2)); } }, null);
		 */
		return new LongPair(getStartTime(dataPointIds),
				getEndTime(dataPointIds));
	}
	...
	static class BatchWriteBehind implements WorkItem {
		private static final ObjectQueue<BatchWriteBehindEntry> ENTRIES = new ObjectQueue<PointValueDao_bak.BatchWriteBehindEntry>();
		...
		static void add(BatchWriteBehindEntry e, DB mongodb) {
			synchronized (ENTRIES) {
				ENTRIES.push(e);
				ENTRIES_MONITOR.setValue(ENTRIES.size());
				if (instances.isEmpty() || ENTRIES.size() > SPAWN_THRESHOLD) {
					BatchWriteBehind bwb = new BatchWriteBehind(mongodb);
					instances.add(bwb);
		...
		private final DB ejt;

		public BatchWriteBehind(DB ejt) {
			this.ejt = ejt;
		}
		...
		public void execute() {
			try {
				...
				while (true) {
					...
					while (true) {
						try {
							// FIXME ejt.update(sql);
							for (int i = 0; i < inserts.length; i++) {
								BatchWriteBehindEntry enter = inserts[i];
								insertPoint(enter.getPointId(), enter.dataType,
										enter.dvalue, enter.time,null);
							}
//							LOG.info("-----------insert point done---------"
//									+ inserts.length);
							break;
						} catch (ConcurrencyFailureException e) {
							if (retries <= 0) {
	 ...
	 * 每10个点一个集合
	 */
	public static final int POINT_COUNT = 1;
	 ...
	 */
	// ok
	public List<Double> getValuesByPointInTimeStamp(long id, long endTime,
			long timestamp) {
		/*
		 * List<Double> values =
		 * queryForList("select pointValue from pointValues_"
		 * +id+" where  ts > ? and  ts < ? ",new Object[]{
		 * endTime-timestamp,endTime },Double.class); return values;
		 */
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (id % POINT_COUNT));
		DBObject ts = new BasicDBObject();
		ts.put("$gt", endTime - timestamp);
		ts.put("$tl", endTime);
		DBObject query = new BasicDBObject();
		query.put("t", ts);
		query.put("i", id);
		...
		while (cursor.hasNext()) {
			DBObject obj = cursor.next();
			pointValues.add((Double) obj.get("v"));
		}
		cursor.close();
		return pointValues;
	}
	...
	public long getEarliestTime() {
		 ...
		 * String sql="select min(ts)as ts from ("; for (int i = 0; i <
		 * dataPointIds.size(); i++) {
		 * sql+="select min(ts) as ts from pointValues_"
		 * +dataPointIds.get(i)+" where dataType = 3";
		 * if(i<dataPointIds.size()-1) sql+=" union all "; } long earliestTime =
		 * ejt.queryForLong(sql+") as tsTable", new Object[0],-1L); return
		 * earliestTime;
		 */
		List<Integer> dataPointIds = new DataPointDao().getDataPointIds();
		return getStartTime(dataPointIds);
	}
	...
	public long getMinTimeByDpid(long from, long to, int dpid) {
		/*
		 * long minTime =
		 * ejt.queryForLong(" select min(ts) from pointvalues_"+dpid
		 * +" where  ts > ? and ts < ? ", new Object[]{from,to},-1L); return
		 * minTime;
		 */
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (dpid % POINT_COUNT));
		...
		DBObject obj = collection
				.findOne(query, new BasicDBObject("t", 1), ts);
		if (obj == null)
			return -1;
		return (Long) obj.get("t");
	}

	public static void insertPoint(int pid, int dataType, double value,
			long time,String svalue) {
		DBObject dbPoint = new BasicDBObject();
		dbPoint.put("e", dataType);
		dbPoint.put("v", svalue!=null?svalue:value);
		dbPoint.put("t", time);
		dbPoint.put("i", pid);
		Integer cid = pid % POINT_COUNT;
		PointsInstance.getInstance().addPoint(cid, dbPoint);
	}

	public long getCountBetween(int dpid,long from, long to) {
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (dpid % POINT_COUNT));
		DBObject between = new BasicDBObject();
		between.put("$gte", from);
		between.put("$lte", to);
		DBObject query = new BasicDBObject();
		query.put("t", between);
		query.put("i", dpid);
	...
	public List<PointValueTime> getPointValuesAt(int pid, long time,
			int count) {
		DBCollection collection = MongoUtil.getColl("pointValues_"
				+ (pid % POINT_COUNT));
		List<PointValueTime> pvts = new ArrayList<PointValueTime>();
		DBObject query1 = new BasicDBObject();
		query1.put("t", new BasicDBObject("$gte",time));
		query1.put("i", pid);
		DBCursor cursor = collection
				.find(query1)
				.sort(new BasicDBObject("t", 1)).limit(count);
		while (cursor.hasNext()) {
			DBObject obj = cursor.next();
			MangoValue value = createMangoValue(obj);
			pvts.add(new PointValueTime(value, (Long) obj.get("t")));
		...
		query2.put("t", new BasicDBObject("$lt",time));
		query2.put("i", pid);
		DBCursor cursor2 = collection
				.find(query2)
				.sort(new BasicDBObject("t", -1)).limit(count);
