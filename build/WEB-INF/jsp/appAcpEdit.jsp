...
<tag:page dwr="AppDatasourceSettingDwr" onload="init">
    ...
    function init() {
        ...
        pointListColumnHeaders.push("<fmt:message key="app.pointname"/>");
         pointListColumnHeaders.push("<fmt:message key="app.point.baseType"/>");
    	...
    	var points = response.data.points;
    	    ...
    	    console.log(point);
    	    	var tr = document.createElement("tr"),
    	    	    ...
    	    	    tdPointType = document.createElement("td"),
    	    	    tdEdit = document.createElement("td");
    	    	    tdName.innerHTML = point.name;
    	    	    tdPointName.innerHTML = point.dataPointVo.extendedName;
    	    	    tdPointType.innerHTML = getTypeName(point.type);
    	    	    var name = "'"+point.name+"'";
    	    	    tdEdit.innerHTML = '<a href="#" onclick="editPoint('+point.id+','+name+','+point.dataPointVo.dataSourceId+','+point.aid+','+point.pointId+','+point.type+')" class="editImg"><tag:img png="icon_ds_edit"
                              title="common.edit" id="editImg'+point.id+'"/></a>';
    	    	    tr.appendChild(tdName);
    	    	    tr.appendChild(tdPointName);
    	    	    tr.appendChild(tdPointType);
    ...
    function getTypeName(type){
		if(type==0)return "";
		if(type==1)return '<fmt:message key="app.basePoint1"/>'; 
		if(type==2)return '<fmt:message key="app.basePoint2"/>'; 
		if(type==3)return '<fmt:message key="app.basePoint3"/>'; 
		if(type==4)return '<fmt:message key="app.basePoint4"/>'; 
		return type;
    }
    ...
    function editPoint(pointId,name,dsid,aid,pid,type) { 
    ...
    function savePoint(){
    	var pid = jQuery("#pid").val(),
    	    ...
    	    pointType = jQuery("#pointTypeId").val(),
    	    aid = jQuery("#hiddenAcpId").val(),
    	    sid =  jQuery("#sortid").val();
        AppDatasourceSettingDwr.savePoint(pid,name,pointId,aid,pointType,sid,function(response){
