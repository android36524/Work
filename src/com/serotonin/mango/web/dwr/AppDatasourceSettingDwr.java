...
public class AppDatasourceSettingDwr extends BaseDwr {
     ...
     * @return
     */
    public DwrResponseI18n savePoint(String pid,String name,String pointId,String aid,String pointType,String sortid){
    	dao = new AppsettingDao();
    	boolean ret = true;
    	DwrResponseI18n response = new DwrResponseI18n();
        if(!"-1".equals(pid)){//修改
        	ret = dao.updatePoint(pid,name,pointId,aid,pointType,sortid);
        }else{//新增
        	ret = dao.addPoint(name,pointId,aid,pointType,sortid);
        }
    ...
    public DwrResponseI18n saveAcpPoints(String dsid,String acpid,String appAcpId){
    	...
    	for(DataPointVO p:points){
    		...
    		dao.addPoint(pointName, p.getId()+"",appAcpId,"0","0");
    	}
    	return response;
    }
