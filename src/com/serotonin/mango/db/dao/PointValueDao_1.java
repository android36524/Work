...
public class PointValueDao_1 extends BaseDao{
...
//FIXME id?		long id= doInsertLong(POINT_VALUE_INSERT_SQL, new Object[] {dataType, dvalue, time });
		long id = 1;
		insertPoint(pointId, dataType, dvalue, time);
...
//ok 
	public List<PointValueTime> getPointValues(int dataPointId, long since) {
		...
		gt.put("$gt", since);
		query.put("ts", gt);
		DBCollection collection = MongoUtil.getColl("pointValues_"+dataPointId);
		List<PointValueTime> pvts = new ArrayList<PointValueTime>();
		DBCursor cursor = collection.find(query).sort(new BasicDBObject("ts", 1));
		while(cursor.hasNext()){
			DBObject obj = cursor.next();
			MangoValue value = createMangoValue(obj);
			pvts.add(new PointValueTime(value,(Long) obj.get("ts")));
		}
		return pvts;
	}
//ok 
	public PointValueTime getPointValuesForWarnCode(int dataPointId, long time) {
		DBObject query = new BasicDBObject();
		query.put("ts", new BasicDBObject("$lte", time));
		DBCollection collection = MongoUtil.getColl("pointValues_"+dataPointId);
		DBCursor cursor = collection.find(query).sort(new BasicDBObject("ts", -1)).limit(1);
		if(cursor.hasNext()){
			DBObject obj = cursor.next();
			MangoValue value = createMangoValue(obj);
			return new PointValueTime(value,(Long) obj.get("ts"));
		}
		return null;
	}
//ok	
...
//ok
	public List<PointValueTime> getLatestPointValues(int dataPointId, int limit) {
		...
		DBCollection collection = MongoUtil.getColl("pointValues_"+dataPointId);
		List<PointValueTime> pvts = new ArrayList<PointValueTime>();
		DBCursor cursor = collection.find().sort(new BasicDBObject("ts", -1)).limit(limit);
		while(cursor.hasNext()){
			DBObject obj = cursor.next();
			MangoValue value = createMangoValue(obj);
			pvts.add(new PointValueTime(value,(Long) obj.get("ts")));
...
//ok
	public List<PointValueTime> getLatestPointValues(int dataPointId,
		...
		List<PointValueTime> pvts = new ArrayList<PointValueTime>();
		DBCursor cursor = collection.find(query).sort(new BasicDBObject("ts", -1)).limit(limit);
		while(cursor.hasNext()){
			DBObject obj = cursor.next();
			MangoValue value = createMangoValue(obj);
			pvts.add(new PointValueTime(value,(Long) obj.get("ts")));
...
//ok
	public void getPointValuesBetween(int dataPointId, long from, long to,MappedRowCallback<PointValueTime> callback) {
		...
		List<IdPointValueTime> idPointValueTimes = new ArrayList<IdPointValueTime>();
			...
			tsQuery.put("lt",to);
			DBCursor cursor = collection.find(new BasicDBObject("ts",tsQuery));
			while(cursor.hasNext()){
				DBObject obj = cursor.next();
				MangoValue value = createMangoValue(obj);
				idPointValueTimes.add(new IdPointValueTime(dataPointId, value, (Long)obj.get("ts")));
			}
	...
	MangoValue createMangoValue(DBObject rs){
		int dataType = (Integer) rs.get("dataType");
		Object obj = rs.get("pointValue");
		MangoValue value;
		switch (dataType) {
		case (DataTypes.NUMERIC):
...
//ok
	public void getPointValuesBetween(List<Integer> dataPointIds, long from,
		...
		for(int i=0;i<dataPointIds.size();i++){
			DBCollection collection = MongoUtil.getColl("pointValues_"+dataPointIds.get(i));
			DBObject tsQuery = new BasicDBObject();
			tsQuery.put("gte",from);
			tsQuery.put("lt",to);
			DBCursor cursor = collection.find(new BasicDBObject("ts",tsQuery));
			while(cursor.hasNext()){
				DBObject obj = cursor.next();
				MangoValue value = createMangoValue(obj);
				idPointValueTimes.add(new IdPointValueTime(dataPointIds.get(i), value, (Long)obj.get("ts")));
			}
		}
	...
	public long deletePointValuesBefore(int dataPointId, long time) {
		DBCollection collection = MongoUtil.getColl("pointValues_"+dataPointId);
		DBObject query = new BasicDBObject("ts",new BasicDBObject("lt",time));
...
//ok
	public long getInceptionDate(int dataPointId) {
		/*return ejt.queryForLong(
				...
				new Object[] {}, -1);*/
		DBCollection collection = MongoUtil.getColl("pointValues_"+dataPointId);
		DBObject ts = new BasicDBObject("ts",1);
		DBObject obj = collection.findOne(null, null, ts);
		if(obj==null)return -1;
		return (Long) obj.get("ts");
	}
//ok
...
//ok
	...
	public boolean getEndDataTime(List<Integer> dataPointIds,long time) {
		...
		for(int i=0;i<dataPointIds.size();i++){
			...
			DBObject obj = collection.findOne(new BasicDBObject("ts",between), null, ts);
			if(obj!=null){
				long ll = (Long) obj.get("ts");
				l = ll>l?ll:l;
				break;
			}
		}
...
//ok
	public LongPair getStartAndEndTime(List<Integer> dataPointIds) {
		...
		return new LongPair(getStartTime(dataPointIds),getEndTime(dataPointIds));
	}
//??????
	...
	static class BatchWriteBehind implements WorkItem {
		private static final ObjectQueue<BatchWriteBehindEntry> ENTRIES = new ObjectQueue<PointValueDao_1.BatchWriteBehindEntry>();
...
//FIXME							ejt.update(sql);
							for(int i=0;i<inserts.length;i++){
								...
								insertPoint(enter.getPointId(),enter.dataType, enter.dvalue, enter.time);
							}
							LOG.info("-----------insert point done---------"+inserts.length);
							break;
						} catch (ConcurrencyFailureException e) {
							if (retries <= 0) {
								LOG
	...
	public List<Double> getValuesByPointInTimeStamp(long id,long endTime,long timestamp){
		...
		return values;*/
		DBCollection collection = MongoUtil.getColl("pointValues_"+id);
		DBObject ts = new BasicDBObject();
		ts.put("gt",endTime-timestamp);
		ts.put("tl", endTime);
		List<Double> pointValues = new ArrayList<Double>();
		DBCursor cursor = collection.find(new BasicDBObject("ts",ts));
		while(cursor.hasNext()){
			DBObject obj = cursor.next();
			pointValues.add((Double) obj.get("pointValue"));
		}
		return pointValues;
	}
	...
	public long getMinTimeByDpid(long from ,long to,int dpid){
		...
		return minTime;*/
		DBCollection collection = MongoUtil.getColl("pointValues_"+dpid);
		DBObject ts = new BasicDBObject("ts",-1);
		DBObject between = new BasicDBObject();
		between.put("gt", from);
		between.put("lt", to);
		DBObject obj = collection.findOne(new BasicDBObject("ts",between), new BasicDBObject("ts",1), ts);
		if(obj==null)return -1;
		return (Long) obj.get("ts");
	}
	
	
	public static void insertPoint(int pid,int dataType,double value,long time){
		DBObject dbPoint = new BasicDBObject();
		DBCollection collection = MongoUtil.getColl("pointValues_"+pid);
		dbPoint.put("dataType", dataType);
		dbPoint.put("pointValue", value);
		dbPoint.put("ts", time);
		collection.insert(dbPoint);
	}
}
